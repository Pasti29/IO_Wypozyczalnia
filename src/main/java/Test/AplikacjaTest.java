...
public class AplikacjaTest {
    ...
    public void dodajWypozyczenie() {
        ...
        for (int i = 0; i < 2; i++) {
            instance.dodajKlienta(dane.imie[i + 5], dane.nazwisko[i + 5], dane.rokUrodzenia[i + 5], dane.nrTelefonu[i + 5]);
            instance.dodajFilm(dane.tytul[i + 5], dane.rezyser[i + 5], dane.gatunek[i + 5], dane.rokProdukcji[i + 5], dane.liczbaDostepnych[i + 5], dane.cena[i + 5]);
            instance.dodajWypozyczenie(dane.imie[i + 5], dane.nazwisko[i + 5], dane.tytul[i + 5], dane.rezyser[i + 5], "2022-01-03");
            Wypozyczenie wyp = instance.szukajWypozyczenia(instance.szukajKlienta(dane.imie[i + 5], dane.nazwisko[i + 5]), instance.szukajFilmu(dane.tytul[i + 5], dane.rezyser[i + 5]), "2022-01-03");
